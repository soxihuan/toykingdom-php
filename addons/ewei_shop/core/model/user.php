...
class Ewei_DShop_User
{
	...
	function getInfo($base64 = false, $debug = false)
	{
		...
		$userinfo = array();
		if (ewei_shop_DEBUG) {
			//$userinfo = array("openid" => "oT-ihv9XGkJbX9owJiLZcZPAJcog", "nickname" => "精品源码之家", "headimgurl" => "", "province" => "云南", "city" => "昆明");
			$userinfo = array("openid" => "fromUser", "nickname" => "fromUser", "headimgurl" => "http://bbs.012wz.com/static/image/common/logo.png", "province" => "广东", "city" => "深圳");
		} else {
			load()->model("mc");
			...
			if ( empty($userinfo["openid"]) && $need_openid && $_GPC["isApp"] != "true" ) { //false) {//  && empty($_GPC["fakeOpenId"])
				die('<!DOCTYPE html>
                <html>
                    <head>
			...
			if(!empty($_GPC["fakeOpenId"])) {
				$userinfo = array("openid" => $_GPC["fakeOpenId"], "nickname" => "窝在家", "headimgurl" => "", "province" => "上海", "city" => "上海");
			}
		}
		if ($base64) {
