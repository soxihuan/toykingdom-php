<?php defined('IN_IA') or exit('Access Denied');?><ul class="nav nav-tabs" >
    ...
    <?php if(cv('order.view.status4')) { ?>
     <li <?php  if($operation == 'display' && $status== '4') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo $this->createWebUrl('order', array('op' => 'display', 'status' => 4))?>">维权申请(<?php  echo $totals['status4'];?>)</a>
    </li>
    <?php  } ?>
     
    <?php if(cv('order.view.status5')) { ?>
    <li <?php  if($operation == 'display' && $status == '5') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo $this->createWebUrl('order', array('op' => 'display', 'status' => 5))?>">完成维权(<?php  echo $totals['status5'];?>)</a>
    </li>
    <?php  } ?>
