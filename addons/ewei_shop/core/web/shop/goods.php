...
if ($operation == "change") {
	...
	$goods = pdo_fetch("select id from " . tablename("ewei_shop_goods") . " where id=:id and uniacid=:uniacid limit 1", array(":uniacid" => $_W["uniacid"], ":id" => $id));
	if (empty($goods)) {
		exit;
	}
		...
		if (is_array($_GPC["cates"])) {
			...
			foreach ($postcates as $pid) {
				if ($cateset["catlevel"] == 3) {
					$tcate = pdo_fetch("select id ,parentid from " . tablename("ewei_shop_category") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $pid, ":uniacid" => $_W["uniacid"]));
					$ccate = pdo_fetch("select id ,parentid from " . tablename("ewei_shop_category") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $tcate["parentid"], ":uniacid" => $_W["uniacid"]));
					$tcates[] = $tcate["id"];
					$ccates[] = $ccate["id"];
					$pcates[] = $ccate["parentid"];
				} else {
					$ccate = pdo_fetch("select id ,parentid from " . tablename("ewei_shop_category") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $pid, ":uniacid" => $_W["uniacid"]));
					$ccates[] = $ccate["id"];
					$pcates[] = $ccate["parentid"];
				}
			}
	...
	if ($pv) {
		$virtual_types = pdo_fetchall("select * from " . tablename("ewei_shop_virtual_type") . " where uniacid=:uniacid order by id asc", array(":uniacid" => $_W["uniacid"]));
	}
	$levels = m("member")->getLevels();
	$details = pdo_fetchall('select detail_logo,detail_shopname,detail_btntext1, detail_btnurl1 ,detail_btntext2,detail_btnurl2,detail_totaltitle from ' . tablename("ewei_shop_goods") . " where uniacid=:uniacid and detail_shopname<>''", array(":uniacid" => $_W["uniacid"]));
