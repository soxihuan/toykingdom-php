...
if ($operation == 'display') {
    ...
    foreach ($list as &$row) {
        $row['follows'] = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_postera_log') . ' where posterid=:posterid and uniacid=:uniacid', array(
            ':posterid' => $row['id'],
            ':uniacid' => $_W['uniacid']
        ));
    }
    ...
    $item = pdo_fetch("SELECT * FROM " . tablename('ewei_shop_postera') . " WHERE id =:id and uniacid=:uniacid limit 1", array(
        ':id' => $id,
        ':uniacid' => $_W['uniacid']
    ));
    if (!empty($item)) {
    ...
    if (checksubmit('submit')) {
        ...
        if ($data['isdefault'] == 1) {
            pdo_update('ewei_shop_postera', array(
                'isdefault' => 0
            ), array(
                'uniacid' => $_W['uniacid'],
                'isdefault' => 1,
        ...
        if (!empty($id)) {
            pdo_update('ewei_shop_postera', $data, array(
                'id' => $id,
                'uniacid' => $_W['uniacid']
            ));
        ...
        message('更新海报成功！', $this->createPluginWebUrl('postera', array(
            'op' => 'display'
        )), 'success');
    }
    ...
    if (!empty($item['goodsid'])) {
        $goods = pdo_fetch('select id,title,thumb,commission_thumb,marketprice,productprice from ' . tablename('ewei_shop_goods') . ' where id=:id and uniacid=:uniacid limit 1', array(
            ':id' => $item['goodsid'],
            ':uniacid' => $_W['uniacid']
        ));
    }
    ...
    if (empty($poster)) {
        message('抱歉，海报不存在或是已经被删除！', $this->createPluginWebUrl('postera', array(
            'op' => 'display'
        )), 'error');
    }
    pdo_delete('ewei_shop_postera', array(
        'id' => $id,
        'uniacid' => $_W['uniacid']
    ));
    pdo_delete('ewei_shop_postera_log', array(
        'posterid' => $id,
        'uniacid' => $_W['uniacid']
    ));
    plog('poster.add', "删除超级海报 ID: {$id} 海报名称: {$poster['title']}");
    message('海报删除成功！', $this->createPluginWebUrl('postera', array(
        'op' => 'display'
    )), 'success');
    ...
    message('海报设置成功！', $this->createPluginWebUrl('postera', array(
        'op' => 'display'
    )), 'success');
}
load()->func('tpl');
