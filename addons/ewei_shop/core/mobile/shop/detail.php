...
$sql = 'select dm.id as memberid,dm.isagent, dm.status,l.* from ' . tablename("ewei_shop_member") . " dm " . " left join " . tablename("ewei_shop_member") . " p on p.id = dm.agentid " . " left join " . tablename("ewei_shop_commission_level") . " l on l.id = dm.agentlevel" . " left join " . tablename("mc_mapping_fans") . "f on f.openid=dm.openid and f.uniacid={$_W["uniacid"]}" . " where dm.uniacid = " . $_W["uniacid"] . " and dm.isagent =1 and dm.id = {$member['id']}";
$res = pdo_fetch($sql);
$is_agent = $res['isagent'];
$ifstatus = $res['status'];

if($is_agent == 1 && $ifstatus == 1){ // only show commission to agent
	$sql = 'select hascommission,commission1_rate,commission2_rate,commission3_rate,commission1_pay,commission2_pay,commission3_pay from ' . tablename('ewei_shop_goods') . ' where id = ' . $goodsid;
	$commissionfee = pdo_fetch($sql);
	$hascommission = $commissionfee['hascommission'];
	if($hascommission == 1){
		$res_display = 1;
		if ($commissionfee['commission1_rate'] != 0){
			$_price1 = $commissionfee['commission1_rate']."%";
			$_price2 = $commissionfee['commission2_rate']."%";
			$_price3 = $commissionfee['commission3_rate']."%";
			$_price = array('1'=>$_price1, '2'=>$_price2, '3'=>$_price3);
		}else{
			$_price1 = "￥".$commissionfee['commission1_pay'];
			$_price2 = "￥".$commissionfee['commission2_pay'];
			$_price3 = "￥".$commissionfee['commission3_pay'];
			$_price = array('1'=>$_price1, '2'=>$_price2, '3'=>$_price3);
		}
		$sql = 'select plugins from '.tablename('ewei_shop_sysset').' where uniacid ='.$uniacid;
		$plugins = pdo_fetch($sql);
		$res = unserialize($plugins['plugins']);
		$res_level= $res['commission']['level'];
	}
	/*$sql = 'select plugins from '.tablename('ewei_shop_sysset').' where uniacid ='.$uniacid;
	$price = $goods['marketprice'];
	$plugins = pdo_fetch($sql);
	$res = unserialize($plugins['plugins']);
	$res_display =$is_agent;
	$_price1 = number_format($price*$res['commission']['commission1']/100, 2);
	$_price2 = number_format($price*$res['commission']['commission2']/100, 2);
	$_price3 = number_format($price*intval($res['commission']['commission3'])/100, 2);
	$_price = array('1'=>$_price1, '2'=>$_price2, '3'=>$_price3);*/
}
...
if ($_W['isajax']) {
	...
	if ($goods['usermaxbuy'] > 0) {
		$order_goodscount = pdo_fetchcolumn('select ifnull(sum(og.total),0)  from ' . tablename('ewei_shop_order_goods') . ' og ' . ' left join ' . tablename('ewei_shop_order') . ' o on og.orderid=o.id ' . ' where og.goodsid=:goodsid and  o.status>=1 and o.openid=:openid  and og.uniacid=:uniacid ', array(':goodsid' => $goodsid, ':uniacid' => $uniacid, ':openid' => $openid));
	...
	$fcount = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member_favorite') . ' where uniacid=:uniacid and openid=:openid and goodsid=:goodsid and deleted=0 ', array(':uniacid' => $uniacid, ':openid' => $openid, ':goodsid' => $goods['id']));
	pdo_query("update " . tablename("ewei_shop_goods") . " set viewcount=viewcount+1 where id=:id and uniacid='{$uniacid}' ", array(":id" => $goodsid));
	$history = pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_member_history') . ' where goodsid=:goodsid and uniacid=:uniacid and openid=:openid and deleted=0 limit 1', array(':goodsid' => $goodsid, ':uniacid' => $uniacid, ':openid' => $openid));
	if ($history <= 0) {
	...
	$ret = array('goods' => $goods, 'followed' => $followed ? 1 : 0, 'followurl' => $followurl, 'followtip' => $followtip, 'saleset' => $saleset, 'pics' => $pics, 'options' => $options, 'specs' => $specs, 'params' => $params, 'commission' => $opencommission, 'commission_text' => $commission_text, 'level' => $level, 'shop' => $shop, 'goodscount' => pdo_fetchcolumn('select count(*) from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and status=1 and deleted=0 ', array(':uniacid' => $uniacid)), 'cartcount' => pdo_fetchcolumn('select sum(total) from ' . tablename('ewei_shop_member_cart') . ' where uniacid=:uniacid and openid=:openid and deleted=0 ', array(':uniacid' => $uniacid, ':openid' => $openid)), 'isfavorite' => $fcount > 0, 'stores' => $stores);
