            ...
            if($userToken){
                ...
                if($goodsList){
                    foreach($goodsList as $key=>$val ){
                        ...
                        $data['data']['goodsList'][$key]['shareUrl'] = URL_QCORD.'index.php?i=2&c=entry&m=ewei_shop&do=shop&p=detail&id='.$val['id'].'&mid='.$mid;
                       ...
                       $url = URL_QCORD."index.php?i=2&c=entry&method=sharesCopyPro&p=commission&m=ewei_shop&do=plugin&isApp=true&fakeOpenId=".$userToken[0]['openid']."&goodsid=".$goodsList[$key]['id'];
                        //返回N张图片
                ...
                if($res){
                    ...
                    if($goodsList){
                        foreach($goodsList as $key=>$val ){
                            ...
                            $data['data']['goodsList'][$key]['shareUrl'] =  URL_QCORD.'index.php?i=2&c=entry&m=ewei_shop&do=shop&p=detail&id='.$val['goods_id'].'&mid='.$mid;
                ...
                if($foundList){
                    foreach($foundList as $key=>$val ){
                        ...
                        if($val['type'] == 1 || empty($val['type'])){
                            ...
                            $data['data'][$key]['shareUrl'] = URL_QCORD.'index.php?i=2&c=entry&m=ewei_shop&do=plugin&&method=myshop&mid='.$mid;
                        ...
                        foreach($array as $arrurlkey => $arrurlval){
                            if($arrurlkey <9){
                                ...
                                if($val['type'] == 1|| empty($val['type'])){
                                    if( $arrurlkey == $last){//将数组最后一张用二维码覆盖
                                         $url = URL_QCORD."index.php?i=2&c=entry&method=sharesCopyPro&p=commission&m=ewei_shop&do=plugin&isApp=true&fakeOpenId=".$userToken[0]['openid']."&goodsid=".$foundList[$key]['goods_id'];
                                    ...
                                    if( $arrurlkey == $last){//将数组最后一张用二维码覆盖
                                        ...
                                        $url = URL_QCORD."index.php?i=2&c=entry&method=sharesCopyPro&p=commission&m=ewei_shop&do=plugin&isApp=true&fakeOpenId=".$userToken[0]['openid'];
                                        $data['data'][$key]['shareImageList'][$arrurlkey]['imageUrl'] = file_get_contents($url);
