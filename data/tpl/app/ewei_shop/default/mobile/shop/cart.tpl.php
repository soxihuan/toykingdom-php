...
.cart_top {height:44px; width:100%;  background:#fff; margin-bottom:8px;}
.cart_top .title {height:44px; width:auto;margin-left:10px; float:left; font-size:16px; line-height:44px; color:#666;}
.cart_top .nav {width:auto;background:#fff; padding:0px 10px; border-right:1px solid #e3e3e3;float:right;margin:12px 0px 0px 16px; color:#666; font-size:14px;}
.cart_top .nav:nth-child(2){border:none;}
.cart_main { height:auto; }
.cart_good {height:80px;  padding:10px 0px; background:#fff;margin-bottom:8px;overflow:hidden;}

.cart_good .ico {height:20px; width:36px; text-align: center; margin-top:25px; float:left;font-size:24px;z-index:2;position: relative;}
...
.cart_good .info .inner .name {height:40px; width:100%; line-height:20px; color:#222121;  font-size:14px;overflow:hidden;}
.cart_good .info .inner .optionsel {height:30px; width:67%; float:left; line-height:30px; font-size:14px; color:#222121;margin-top: 5px;}
.cart_good .info .inner .optionsel .content { float:left;width:100%;overflow:hidden;height:30px;}
.cart_good .info .inner .optionsel .content .cinner {}
...
.cart_good .other .price,.cart_good .other .price1 { width:100%;height:20px;text-align:right;color:#e60011;font-size:14px; }
...
.cart_pay {float:left;height:54px; width:100%;background:#fff; border-top:1px solid #e1e1e1; position:fixed; bottom:0px; left:0px;padding-left:8px;box-sizing: border-box;z-index:9;}
.cart_pay .all {height:54px; width:100px;  float:left; line-height:50px; font-size:14px; color:#222121; }
.cart_pay .all i { font-size:24px;margin-top:15px; float:left;}
.cart_pay .all .t { float:left;margin-left:5px;}
.cart_pay .paysub {height:100%; width:110px;background:#F4D500; float:right; /* border-radius:5px; */ font-size:16px; line-height:55px; color:#222121; /* margin-top:7px; */text-align:center;}
.cart_pay .disabled { background:#ccc;}
.cart_pay .text {height:40px; width:auto; float:right; line-height:20px; color:#222121; font-size:14px; margin:7px 20px 0px;}
.cart_pay .text span {color:#e60011;}

.cart_count {height:40px; padding:0 3px; text-align:right; color:#bbb; font-size:12px; line-height:40px;padding-bottom:55px;}
...
.on-all-choose{display:inline-block;height:20px;width:20px;background:url('../addons/ewei_shop/template/mobile/default/static/images/choose.png') no-repeat center center;background-size:contain;}
.no-choose{display:inline-block;height:20px;width:20px;background:url('../addons/ewei_shop/template/mobile/default/static/images/not-to-choose.png') no-repeat center center;background-size:contain;}
.text span.withoutway{font-size:10px;-webkit-transform:scale(0.9);transform:scale(0.9);color:#222111;display:block;text-align:center;}
.theDown{height:8px;width:12px;display:inline-block;background:url('../addons/ewei_shop/template/mobile/default/static/images/down-cart-arrow.png') no-repeat center center;background-size:contain;display:none;}

.cart_good .other .num {position:absolute;left:-37px;top:46px;}
.cart_good .other .num div{display:inline-block;height:30px;width:37px;}
.cart_good .other .num .leftnav  {float:left;background:url('../addons/ewei_shop/template/mobile/default/static/images/icon_left_2.png') no-repeat center center;background-size:contain;}
.cart_good .other .num .rightnav {float:right;background:url('../addons/ewei_shop/template/mobile/default/static/images/icon_right_2.png') no-repeat 0px center;background-size:contain;}

.cart_good .other .num input{width:43px;text-align:center;border:none;height:30px;line-height:30px;float:left;-webkit-appearance:none;}

.cart_good .other .num input{background:url('../addons/ewei_shop/template/mobile/default/static/images/icon_centre.png') no-repeat center center;background-size:45px 30px;padding:0;}
...
<div class="cart_top">
    ...
    <div class="cart_main">
          <%each list as value%>
             <div class="cart_good" 
                 ...
                 <div class="ico"><i class="on-all-choose" style="color:#0C9;"></i></div>
            	      ...
            	      <div class="name" onclick="location.href='<?php  echo $this->createMobileUrl('shop/detail')?>&id=<%value.goodsid%>'"><%value.title%></div>
                        <%if value.optionid!='0'%>
                         <div class="optionsel">
                            ...
                            <div class='content'>
                                <div style="display:inline-block;float:left;">规格:</div>
                                <div style="display:inline-block;float:left;width:70px;white-space:nowrap;" class="cinner"><%if optiontitle!=''%><%value.optiontitle%><%else%>未选规格<%/if%></div>
                            </div>
                            <div class="cico"><i class="theDown"></i></div>
                        </div>
                        <%/if%>
    ...
    <div class="cart_pay">
        <div class="all" sel="1"><i class="on-all-choose" style="color:#0C9;"></i> <span class='t'>全选</span></div>
        <div class="paysub <%if total<=0%>disabled<%/if%>">结算(<span class='total'><%total%></span>)</div>
        <div class="text">合计：<span style="font-weight:500;">￥<span  class='totalprice'><%totalprice%></span></span><br><span class="withoutway" class="withoutway" style="">（不含运费）</span></div>
    </div>
</script>
<script language="javascript">
    ...
    function setSelect(obj, sel,isall){
            if(sel=='1'){
                         obj.find('i').addClass('no-choose').removeClass('on-all-choose').css('color', '#999');
                    }
                    else{
                         obj.find('i').removeClass('no-choose').addClass('on-all-choose').css('color', '#0c9');
                    }
                    sel =sel==1?0:1;
                    if(!isall){
          ...
          $('.good_choose .other').css('max-height',bh-175);
           ...
           $('.shownum').blur(function(){
               ...
               var stock = parseInt( $(this).closest('.cart_good').data('stock'));
                 var MyCloudId= $(this).closest('.cart_good').attr('data-goodsid');
                ...
                var sureNum = parseInt($(this).val());
                var num  = 0;
                $('.cart_good').each(function(index, el) {
                  if ($(this).attr('data-goodsid') == MyCloudId) {
                      num = Number($(this).find('.shownum').val())+num;
                ...
                var num1=0;
                $(this).closest('.cart_good').siblings('.cart_good').each(function(index, el) {
                      if ($(this).attr('data-goodsid') == MyCloudId) {
                      num1 = Number($(this).find('.shownum').val())+num1;

                  };
                });
                var onlyBuy = maxbuy - num1;
            ...
            $('.leftnav').click(function(){
             ...
             $('.rightnav').click(function(){
                var maxbuy = parseInt( $(this).closest('.cart_good').data('maxbuy'));
                var stock = parseInt( $(this).closest('.cart_good').data('stock'));
                var numNow = Number($(this).prev().val());
			...
			if(ret.no==''){
			   ...
			   $("#optionid").val(optionid); 
				...
				if(ret.no==''){
		     	...
		     	$("#option_price").html(marketprice);	
		     	$("#option_stock").html(stock);
                // $('.cart_good[data-cartid=' + id + ']').find('.price').html(marketprice)
                        ...
                        $('.sub').click(function(){
                            ...
                            core.json('shop/cart',{'op':'setoption',id:id,goodsid:goodsid,optionid:optionid},function(json){
                                  if(json.status==1){
                                       ...
                                       // alert(json.result.optionid)

                                       // $('.cart_good[data-cartid=' + id + ']').find('.price').html(json.result.productprice)

                                       // $('.cart_good[data-cartid=' + id + ']').find('.price1')html(json.result.marketprice)
                                 }
                                 else{
