...
if ($level >= 1) {
	...
	$level1_orders = pdo_fetchall("select commission1,o.id,o.createtime,o.price,og.commissions,o.status from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on og.orderid=o.id " . " where o.uniacid=:uniacid and o.agentid=:agentid {$condition} and og.status1>=0 and og.nocommission=0  ORDER BY  o.createtime", array(":uniacid" => $_W["uniacid"], ":agentid" => $member["id"]));
...
if ($level >= 2) {
	if ($level1 > 0) {
		$level2_orders = pdo_fetchall("select commission2 ,o.id,o.createtime,o.price,og.commissions,o.status   from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on og.orderid=o.id " . " where o.uniacid=:uniacid and o.agentid in( " . implode(",", array_keys($member["level1_agentids"])) . ")  {$condition}  and og.status2>=0 and og.nocommission=0 ORDER BY  o.createtime", array(":uniacid" => $_W["uniacid"]));
		foreach ($level2_orders as $o) {
            ...
            foreach ($ordersTwo as &$or2) {
                if ($or2["id"] == $o["id"] && $or2["level"] == 2) {
                    $or2["commission"] += $commission_ok;
                    $hasorder = true;
                    break;
                }
            }
			...
			if (!$hasorder) {
				...
				$ordersTwo[] =array("id" => $o["id"], "commission" => $commission_ok, "createtime" => $o["createtime"], "level" => 2);
			}
            if($o["status"] != 0){
			$commissioncount += $commission_ok;
           }
...
if ($level >= 3) {
	if ($level2 > 0) {
		$level3_orders = pdo_fetchall("select commission3 ,o.id,o.createtime,o.price,og.commissions,o.status   from " . tablename("ewei_shop_order_goods") . " og " . " left join  " . tablename("ewei_shop_order") . " o on og.orderid=o.id " . " where o.uniacid=:uniacid and o.agentid in( " . implode(",", array_keys($member["level2_agentids"])) . ")  {$condition} and og.status3>=0 and og.nocommission=0  ORDER BY  o.createtime", array(":uniacid" => $_W["uniacid"]));
		foreach ($level3_orders as $o) {
            ...
            foreach ($ordersThree as &$or3) {
                if ($or3["id"] == $o["id"] && $or3["level"] == 2) {
                    $or3["commission"] += $commission_ok;
                    $hasorder = true;
                    break;
                }
            }
			...
			if (!$hasorder) {
				...
				$ordersThree[] = array("id" => $o["id"], "commission" => $commission_ok, "createtime" => $o["createtime"], "level" => 3);
			}
             if($o["status"] != 0){
			    $commissioncount += $commission_ok;
             }
...
if ($_W["isajax"]) {
	...
	// 新加电话搜索
	$thisOP = $_GPC["op"];
	if ($thisOP == 'selectNumber') {
		$telNum = $_GPC["telephone"];
		// $pindex = 1;
		// $psize = 200;
		$pageorders = array();
		$orders1= array_reverse($orders);
		$orderids = array();
		...
		$list = array();
		if (!empty($orderids) && !empty($telNum)){
		$list = pdo_fetchall("select id,ordersn,openid,createtime,status,expresssn,expresscom,express,remarkForConsumer,address from " . tablename("ewei_shop_order") . "  where uniacid ={$_W["uniacid"]} and id in ( " . implode(",", array_keys($orderids)) . ")  and address LIKE '%".$telNum."%' order by id desc");
	...
	$commissionLevel = $_GPC["MyLevel"];
	if ( $commissionLevel == 1 ) {
		$pindex = max(1, intval($_GPC["page1"]));
		unset($orders);
		$orders = $ordersOne;

	}else if( $commissionLevel == 2 ){
		$pindex = max(1, intval($_GPC["page2"]));
		$orders = $ordersTwo;
	}
	...
	if (!empty($orderids)) {
		$list = pdo_fetchall("select id,ordersn,openid,createtime,status,expresssn,expresscom,express,remarkForConsumer,address from " . tablename("ewei_shop_order") . "  where uniacid ={$_W["uniacid"]} and id in ( " . implode(",", array_keys($orderids)) . ") order by id desc");
...
include $this->template("orderCopy");