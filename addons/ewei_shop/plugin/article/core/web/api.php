...
if ($_W["isajax"] && $_W["ispost"]) {
	if ($apido == "delarticle") {
		...
		if (!empty($article)) {
			pdo_delete("ewei_shop_article", array("id" => $aid));
			...
			$article_log = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_article_log") . " WHERE aid=:aid and uniacid=:uniacid ", array(":aid" => $article["id"], ":uniacid" => $_W["uniacid"]));
			if (!empty($article_log)) {
				pdo_delete("ewei_shop_article_log", array("aid" => $article["id"]));
			}
			$article_share = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_article_share") . " WHERE aid=:aid and uniacid=:uniacid ", array(":aid" => $article["id"], ":uniacid" => $_W["uniacid"]));
			if (!empty($article_log)) {
				pdo_delete("ewei_shop_article_share", array("aid" => $article["id"]));
			}
		...
		$cid = intval($_GPC["cid"]);
		$category = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_article_category") . " WHERE id=:cid and uniacid=:uniacid limit 1 ", array(":cid" => $cid, ":uniacid" => $_W["uniacid"]));
		if (!empty($category)) {
			$articles = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_article") . " WHERE article_category=:cid and uniacid=:uniacid ", array(":cid" => $cid, ":uniacid" => $_W["uniacid"]));
			if (!empty($articles)) {
				die(json_encode(array("result" => "error2")));
			} else {
				pdo_delete("ewei_shop_article_category", array("id" => $cid));
				die(json_encode(array("result" => "success")));
			}
		} else {
			die(json_encode(array("result" => "error")));
		...
		$cname = ($_GPC["cname"]);
		if (!empty($cname)) {
			$cates = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_article_category") . " WHERE category_name=:cname and id<>:cid and uniacid=:uniacid limit 1 ", array(":cid" => $cid, ":cname" => $cname, ":uniacid" => $_W["uniacid"]));
			if (!empty($cates)) {
			...
			if (empty($cid)) {
				...
				pdo_insert("ewei_shop_article_category", $arr);
				$insertid = pdo_insertid();
				die(json_encode(array("result" => "success-add", "cid" => $insertid, "cname" => $cname)));
			} else {
				ca("article.cate.editcate");
				pdo_update("ewei_shop_article_category", $arr, array("id" => $cid));
		...
		if (is_array($data)) {
			...
			if (!empty($arr["id"])) {
				$articlekeyword = pdo_fetchcolumn("SELECT article_keyword FROM " . tablename("ewei_shop_article") . " WHERE id=:aid and uniacid=:uniacid limit 1 ", array(":aid" => $arr["id"], ":uniacid" => $_W["uniacid"]));
			...
			$arr["article_endtime"] = strtotime($arr["article_endtime"]);
			$arr["resp_img"] = save_media($arr["resp_img"]);
			if (empty($arr["id"])) {
				$arr["article_date"] = date("Y-m-d H:i:s");
				ca("article.page.add");
				pdo_insert("ewei_shop_article", $arr);
				$aid = pdo_insertid();
				$desc = "insert";
			} else {
				ca("article.page.edit");
				pdo_update("ewei_shop_article", $arr, array("id" => $arr["id"]));
				$aid = $arr["id"];
				$desc = "update";
			}
			$rule = pdo_fetch("select * from " . tablename("rule") . " where uniacid=:uniacid and module=:module and name=:name  limit 1", array(":uniacid" => $_W["uniacid"], ":module" => "ewei_shop", ":name" => "ewei_shop:article:" . $arr["id"]));
			if (empty($rule)) {
				$rule_data = array("uniacid" => $_W["uniacid"], "name" => "ewei_shop:article:" . $arr["id"], "module" => "ewei_shop", "displayorder" => 0, "status" => 1);
				pdo_insert("rule", $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array("uniacid" => $_W["uniacid"], "rid" => $rid, "module" => "ewei_shop", "content" => trim($arr["article_keyword"]), "type" => 1, "displayorder" => 0, "status" => 1);
		...
		$kw = $_GPC["kw"];
		$goods = pdo_fetchall("SELECT id,title,productprice,marketprice,thumb,hasoption FROM " . tablename("ewei_shop_goods") . " WHERE uniacid= :uniacid and status=1 and deleted=0 AND title LIKE :title ", array(":title" => "%{$kw}%", ":uniacid" => $_W["uniacid"]));
		...
		$articles = pdo_fetchall("SELECT a.id,a.article_title,a.article_category,c.category_name FROM " . tablename("ewei_shop_article") . " a left join " . tablename("ewei_shop_article_category") . " c on c.id=a.article_category WHERE a.uniacid= :uniacid " . $where . " and a.article_title LIKE :title ", array(":title" => "%{$keyword}%", ":uniacid" => $_W["uniacid"]));
		...
		if (!empty($arr)) {
			$rule = pdo_fetch("select * from " . tablename("rule") . " where uniacid=:uniacid and module=:module and name=:name limit 1", array(":uniacid" => $_W["uniacid"], ":module" => "cover", ":name" => "ewei_shop文章营销入口设置"));
			if (!empty($rule)) {
			...
			$rule_data = array("uniacid" => $_W["uniacid"], "name" => "ewei_shop文章营销入口设置", "module" => "cover", "displayorder" => 0, "status" => 1);
			if (empty($rule)) {
				pdo_insert("rule", $rule_data);
				$rid = pdo_insertid();
			} else {
