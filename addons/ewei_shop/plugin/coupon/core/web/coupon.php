...
if ($operation == "display") {
	...
	if (checksubmit("submit")) {
		...
		if (!empty($id)) {
			if (!empty($data["pwdkey"])) {
				$pwdkey = pdo_fetchcolumn("SELECT pwdkey FROM " . tablename("ewei_shop_coupon") . " WHERE id=:id and uniacid=:uniacid limit 1 ", array(":id" => $id, ":uniacid" => $_W["uniacid"]));
		...
		$key = "ewei_shop:coupon:" . $id;
		$rule = pdo_fetch("select * from " . tablename("rule") . " where uniacid=:uniacid and module=:module and name=:name  limit 1", array(":uniacid" => $_W["uniacid"], ":module" => "ewei_shop", ":name" => $key));
		if (!empty($data["pwdkey"])) {
			if (empty($rule)) {
				$rule_data = array("uniacid" => $_W["uniacid"], "name" => $key, "module" => "ewei_shop", "displayorder" => 0, "status" => $data["pwdopen"]);
				pdo_insert("rule", $rule_data);
				$rid = pdo_insertid();
				$keyword_data = array("uniacid" => $_W["uniacid"], "rid" => $rid, "module" => "ewei_shop", "content" => $data["pwdkey"], "type" => 1, "displayorder" => 0, "status" => $data["pwdopen"]);
	...
	$id = intval($_GPC["id"]);
	$item = pdo_fetch("SELECT id,couponname FROM " . tablename("ewei_shop_coupon") . " WHERE id =:id and uniacid=:uniacid limit 1", array(":id" => $id, ":uniacid" => $_W["uniacid"]));
	if (empty($item)) {
		message("抱歉，优惠券不存在或是已经被删除！", $this->createPluginWebUrl("coupon/coupon", array("op" => "display")), "error");
	}
	pdo_delete("ewei_shop_coupon", array("id" => $id, "uniacid" => $_W["uniacid"]));
	$couponids = pdo_fetchall("select id from " . tablename("ewei_shop_coupon") . " where uniacid=:uniacid", array(":uniacid" => $_W["uniacid"]), "id");
	if (!empty($couponids)) {
		pdo_query("delete from " . tablename("ewei_shop_coupon_data") . " where couponid not in (" . implode(",", array_keys($couponids)) . ") and uniacid=:uniacid", array(":uniacid" => $_W["uniacid"]));
	}
	pdo_delete("ewei_shop_coupon_data", array("couponid" => $id, "uniacid" => $_W["uniacid"]));
