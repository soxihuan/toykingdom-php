...
if ($operation == "display") {
	...
	$list = pdo_fetchall("SELECT s.*,m.nickname,m.avatar,m.mobile,m.realname,store.storename FROM " . tablename("ewei_shop_saler") . "  s " . " left join " . tablename("ewei_shop_member") . " m on s.openid=m.openid and m.uniacid = s.uniacid " . " left join " . tablename("ewei_shop_store") . " store on store.id=s.storeid " . " WHERE s.uniacid = '{$_W["uniacid"]}' ORDER BY id asc");
