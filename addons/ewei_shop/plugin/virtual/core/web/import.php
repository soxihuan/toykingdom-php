...
if ($operation == 'temp') {
	...
	foreach ($datas as $d) {
		$olddata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_virtual_data') . ' WHERE pvalue=:pvalue and typeid=:typeid and uniacid=:uniacid ', array(':pvalue' => $d['pvalue'], ':typeid' => $_GPC['typeid'], ':uniacid' => $_W['uniacid']));
		if (empty($olddata)) {
	...
	if (!empty($noinsert)) {
		...
		message("导入成功！" . $tip, $this->createPluginWebUrl('virtual/data', array('typeid' => $_GPC['typeid'])));
	}
}