...
global $_W, $_GPC;
if ($_W['isajax']){
	$operation1 = !empty($_GPC["op"]) ? $_GPC["op"] : "display";
	if ($operation1 == 'MyChangeTip') {
		$MyVal = $_GPC["MyChangeTip"];
		$mysn = $_GPC["mysnn"];
		pdo_update("ewei_shop_order", array("remark" => $MyVal), array("ordersn" => $mysn));
		show_json(1,$mysn);
		include $this->template("web/shop/list");
	}
}
...
if ($operation == "display") {
	...
	if ($to == "confirmpay") {
		...
		if (!empty($refundid)) {
			$refund = pdo_fetch("select * from " . tablename("ewei_shop_order_refund") . " where id=:id and uniacid=:uniacid  limit 1", array(":id" => $refundid, ":uniacid" => $_W["uniacid"]));
		} else {
			die("未找到退款申请.");
			exit;
		}
		...
		// $list[];
		$list;
		for ($i=0; $i < count($arr['result']['list']); $i++) { 
			$list[$i]= array("time" => $arr['result']['list'][$i]['time'], "status" => $arr['result']['list'][$i]['status']);
		}
		// show_json(1,$list);
		// var_dump($arr);
		// $len = count($arr);
...
function getList($_var_2, $_var_3)
{
		...
		$result = m('express')->curlOpen($url, array('post'=>$post));
		// $result =m('MyFunction')->curlOpen($url, array('post'=>$post));
...
function order_list_confirmsend($_var_21)
{
	...
	if (!empty($_var_21["refundid"])) {
		...
		if (!empty($_var_22)) {
			...
			pdo_update("ewei_shop_order", array("refundstate" => 0), array("id" => $_var_21["id"]));
		}
	}
...
function order_list_close($_var_21)
{
	...
	$_var_23 = time();
	if ($_var_21["refundstate"] > 0 && !empty($_var_21["refundid"])) {
		$_var_26 = array();
		$_var_26["status"] = -1;
		$_var_26["refundtime"] = $_var_23;
		pdo_update("ewei_shop_order_refund", $_var_26, array("id" => $_var_21["refundid"], "uniacid" => $_W["uniacid"]));
	}
	pdo_update("ewei_shop_order", array("status" => -1, "refundstate" => 0, "canceltime" => $_var_23, "remark" => $_var_21["remark"] . "\r\n" . $_GPC["remark"]), array("id" => $_var_21["id"], "uniacid" => $_W["uniacid"]));
	if ($_var_21["deductcredit"] > 0) {
		$_var_27 = m("common")->getSysset("shop");
		m("member")->setCredit($_var_21["openid"], "credit1", $_var_21["deductcredit"], array('0', $_var_27["name"] . "购物返还抵扣积分 积分: {$_var_21["deductcredit"]} 抵扣金额: {$_var_21["deductprice"]} 订单号: {$_var_21["ordersn"]}"));
	}
	if (p("coupon") && !empty($_var_21["couponid"])) {
...
function order_list_refund($_var_21)
{
	...
	ca("order.op.refund");
	$_var_27 = m("common")->getSysset("shop");
	if (empty($_var_21["refundstate"])) {
		message("订单未申请退款，不需处理！");
	}
	$_var_22 = pdo_fetch("select * from " . tablename("ewei_shop_order_refund") . " where id=:id and (status=0 or status>1) order by id desc limit 1", array(":id" => $_var_21["refundid"]));
	if (empty($_var_22)) {
		pdo_update("ewei_shop_order", array("refundstate" => 0), array("id" => $_var_21["id"], "uniacid" => $_W["uniacid"]));
	...
	$_var_28 = intval($_GPC["refundstatus"]);
	$_var_29 = trim($_GPC["refundcontent"]);
	$_var_23 = time();
	$_var_26 = array();
	$_var_30 = $_W["uniacid"];
	if ($_var_28 == 0) {
		...
		$_var_31 = $_GPC["raid"];
		$_var_32 = trim($_GPC["message"]);
		if ($_var_31 == 0) {
			$_var_33 = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where isdefault=1 and uniacid=:uniacid limit 1", array(":uniacid" => $_var_30));
		} else {
			$_var_33 = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where id=:id and uniacid=:uniacid limit 1", array(":id" => $_var_31, ":uniacid" => $_var_30));
		}
		if (empty($_var_33)) {
			$_var_33 = pdo_fetch("select * from " . tablename("ewei_shop_refund_address") . " where uniacid=:uniacid order by id desc limit 1", array(":uniacid" => $_var_30));
		}
		unset($_var_33["uniacid"]);
		unset($_var_33["openid"]);
		unset($_var_33["isdefault"]);
		unset($_var_33["deleted"]);
		$_var_33 = iserializer($_var_33);
		$_var_26["reply"] = '';
		$_var_26["refundaddress"] = $_var_33;
		$_var_26["refundaddressid"] = $_var_31;
		$_var_26["message"] = $_var_32;
		if (empty($_var_22["operatetime"])) {
			$_var_26["operatetime"] = $_var_23;
		}
		if ($_var_22["status"] != 4) {
			$_var_26["status"] = 3;
		}
		pdo_update("ewei_shop_order_refund", $_var_26, array("id" => $_var_21["refundid"]));
		m("notice")->sendOrderMessage($_var_21["id"], true);
	} else if ($_var_28 == 5) {
		$_var_26["rexpress"] = $_GPC["rexpress"];
		$_var_26["rexpresscom"] = $_GPC["rexpresscom"];
		$_var_26["rexpresssn"] = trim($_GPC["rexpresssn"]);
		$_var_26["status"] = 5;
		if ($_var_22["status"] != 5 && empty($_var_22["returntime"])) {
			$_var_26["returntime"] = $_var_23;
		}
		pdo_update("ewei_shop_order_refund", $_var_26, array("id" => $_var_21["refundid"]));
		m("notice")->sendOrderMessage($_var_21["id"], true);
	} else if ($_var_28 == 10) {
		$_var_34["status"] = 1;
		$_var_34["refundtime"] = $_var_23;
		pdo_update("ewei_shop_order_refund", $_var_34, array("id" => $_var_21["refundid"], "uniacid" => $_var_30));
		$_var_35 = array();
		$_var_35["refundstate"] = 0;
		$_var_35["status"] = 3;
		$_var_35["refundtime"] = $_var_23;
		pdo_update("ewei_shop_order", $_var_35, array("id" => $_var_21["id"], "uniacid" => $_var_30));
		m("notice")->sendOrderMessage($_var_21["id"], true);
	} else if ($_var_28 == 1) {
		$_var_8 = $_var_21["ordersn"];
		if (!empty($_var_21["ordersn2"])) {
			$_var_36 = sprintf("%02d", $_var_21["ordersn2"]);
			$_var_8 .= "GJ" . $_var_36;
		}
		$_var_37 = $_var_22["applyprice"];
		$_var_38 = pdo_fetchall("SELECT g.id,g.credit, o.total,o.realprice FROM " . tablename("ewei_shop_order_goods") . " o left join " . tablename("ewei_shop_goods") . " g on o.goodsid=g.id " . " WHERE o.orderid=:orderid and o.uniacid=:uniacid", array(":orderid" => $_var_21["id"], ":uniacid" => $_var_30));
		$_var_39 = 0;
		foreach ($_var_38 as $_var_40) {
			$tmpcredit = trim($_var_40["credit"]);
			if (!empty($tmpcredit)) {
				if (strexists($tmpcredit, "%")) {
					$_var_39 += intval(floatval(str_replace("%", '', $tmpcredit)) / 100 * $_var_40["realprice"]);
				} else {
					$_var_39 += intval($_var_40["credit"]) * $_var_40["total"];
				}
			}
		}
		$_var_41 = 0;
		if ($_var_21["paytype"] == 1) {
			m("member")->setCredit($_var_21["openid"], "credit2", $_var_37, array(0, $_var_27["name"] . "退款: {$_var_37}元 订单号: " . $_var_21["ordersn"]));
			$_var_42 = true;
		} else if ($_var_21["paytype"] == 21) {
			$_var_37 = round($_var_37 - $_var_21["deductcredit2"], 2);
			$_var_42 = m("finance")->refund($_var_21["openid"], $_var_8, $_var_22["refundno"], $_var_21["price"] * 100, $_var_37 * 100);
			$_var_41 = 2;
		} else {
			if ($_var_37 < 1) {
			...
			$_var_37 = round($_var_37 - $_var_21["deductcredit2"], 2);
			$_var_42 = m("finance")->pay($_var_21["openid"], 1, $_var_37 * 100, $_var_22["refundno"], $_var_27["name"] . "退款: {$_var_37}元 订单号: " . $_var_21["ordersn"]);
			$_var_41 = 1;
		}
		if (is_error($_var_42)) {
			message($_var_42["message"], '', "error");
		}
		if ($_var_39 > 0) {
			m("member")->setCredit($_var_21["openid"], "credit1", -$_var_39, array(0, $_var_27["name"] . "退款扣除积分: {$_var_39} 订单号: " . $_var_21["ordersn"]));
		}
		if ($_var_21["deductcredit"] > 0) {
			m("member")->setCredit($_var_21["openid"], "credit1", $_var_21["deductcredit"], array('0', $_var_27["name"] . "购物返还抵扣积分 积分: {$_var_21["deductcredit"]} 抵扣金额: {$_var_21["deductprice"]} 订单号: {$_var_21["ordersn"]}"));
		}
		if (!empty($_var_41)) {
			if ($_var_21["deductcredit2"] > 0) {
				m("member")->setCredit($_var_21["openid"], "credit2", $_var_21["deductcredit2"], array('0', $_var_27["name"] . "购物返还抵扣余额 积分: {$_var_21["deductcredit2"]} 订单号: {$_var_21["ordersn"]}"));
			}
		}
		$_var_26["reply"] = '';
		$_var_26["status"] = 1;
		$_var_26["refundtype"] = $_var_41;
		$_var_26["price"] = $_var_37;
		$_var_26["refundtime"] = $_var_23;
		if (empty($_var_22["operatetime"])) {
			$_var_26["operatetime"] = $_var_23;
		}
		pdo_update("ewei_shop_order_refund", $_var_26, array("id" => $_var_21["refundid"]));
		m("notice")->sendOrderMessage($_var_21["id"], true);
		pdo_update("ewei_shop_order", array("refundstate" => 0, "status" => -1, "refundtime" => $_var_23), array("id" => $_var_21["id"], "uniacid" => $_var_30));
		foreach ($_var_38 as $_var_40) {
			$_var_43 = pdo_fetchcolumn("select ifnull(sum(total),0) from " . tablename("ewei_shop_order_goods") . " og " . " left join " . tablename("ewei_shop_order") . " o on o.id = og.orderid " . " where og.goodsid=:goodsid and o.status>=1 and o.uniacid=:uniacid limit 1", array(":goodsid" => $_var_40["id"], ":uniacid" => $_var_30));
			pdo_update("ewei_shop_goods", array("salesreal" => $_var_43), array("id" => $_var_40["id"]));
		}
		plog("order.op.refund", "订单退款 ID: {$_var_21["id"]} 订单号: {$_var_21["ordersn"]}");
	} else if ($_var_28 == -1) {
		pdo_update("ewei_shop_order_refund", array("reply" => $_var_29, "status" => -1, "endtime" => $_var_23), array("id" => $_var_21["refundid"]));
		m("notice")->sendOrderMessage($_var_21["id"], true);
		plog("order.op.refund", "订单退款拒绝 ID: {$_var_21["id"]} 订单号: {$_var_21["ordersn"]} 原因: {$_var_29}");
		pdo_update("ewei_shop_order", array("refundstate" => 0), array("id" => $_var_21["id"], "uniacid" => $_var_30));
	} else if ($_var_28 == 2) {
		$_var_41 = 2;
		$_var_26["reply"] = '';
		$_var_26["status"] = 1;
		$_var_26["refundtype"] = $_var_41;
		$_var_26["price"] = $_var_22["applyprice"];
		$_var_26["refundtime"] = $_var_23;
		if (empty($_var_22["operatetime"])) {
			$_var_26["operatetime"] = $_var_23;
		}
