...
if ($operation == "display") {
	if (!empty($_GPC["displayorder"])) {
		...
		message("分类排序更新成功！", $this->createPluginWebUrl("article", array("op" => "display")), "success");
	}
	...
	$goodcates = pdo_fetchall("SELECT id,name,parentid FROM " . tablename("ewei_shop_category") . " WHERE enabled=:enabled and uniacid= :uniacid  ", array(":uniacid" => $_W["uniacid"], ":enabled" => "1"));
	$articles = pdo_fetchall("SELECT id,article_title,article_category FROM " . tablename("ewei_shop_article") . " WHERE uniacid=:uniacid ", array(":uniacid" => $_W["uniacid"]));
	$article_sys = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_article_sys") . " WHERE uniacid=:uniacid limit 1 ", array(":uniacid" => $_W["uniacid"]));
	$designer = p("designer");
	if ($designer) {
		$diypages = pdo_fetchall("SELECT id,pagetype,setdefault,pagename FROM " . tablename("ewei_shop_designer") . " WHERE uniacid=:uniacid order by setdefault desc  ", array(":uniacid" => $_W["uniacid"]));
	}
	...
	$pindex = max(1, intval($page));
	$psize = 15;
	$list = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_article_category") . " WHERE category_name LIKE :name and uniacid=:uniacid order by id desc LIMIT " . ($pindex - 1) * $psize . "," . $psize, array(":name" => "%{$kw}%", ":uniacid" => $_W["uniacid"]));
	$total = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("ewei_shop_article_category") . " WHERE category_name LIKE :name and uniacid=:uniacid ", array(":name" => "%{$kw}%", ":uniacid" => $_W["uniacid"]));
	$pager = pagination($total, $pindex, $psize);
	$categorynum = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename("ewei_shop_article_category") . " WHERE uniacid= :uniacid ", array(":uniacid" => $_W["uniacid"]));
	...
	if (!empty($aid)) {
		...
		if ($operation == "list_read") {
			$list_reads = pdo_fetchall("SELECT l.id,l.aid,l.read,l.like,u.nickname,u.uid FROM " . tablename("ewei_shop_article_log") . " l left join " . tablename("ewei_shop_member") . " u on u.openid=l.openid WHERE l.aid=:aid and l.uniacid=:uniacid order by l.id desc limit " . ($pindex - 1) * $psize . "," . $psize, array(":aid" => $aid, ":uniacid" => $_W["uniacid"]));
	...
	$articles = pdo_fetchall("SELECT id,article_title FROM " . tablename("ewei_shop_article") . " WHERE uniacid=:uniacid ", array(":uniacid" => $_W["uniacid"]));
	$kw = $_GPC["keyword"];
	$aid = intval($_GPC["aid"]);
	$cid = $_GPC["cid"];
	$where = '';
