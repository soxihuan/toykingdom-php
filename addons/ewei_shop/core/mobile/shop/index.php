...
if ($_W["isajax"]) {
	if ($operation == "index") {
		...
		$category = pdo_fetchall("select id,name,thumb,parentid,level from " . tablename("ewei_shop_category") . " where uniacid=:uniacid and ishome=1 and enabled=1 order by displayorder desc", array(":uniacid" => $uniacid));
