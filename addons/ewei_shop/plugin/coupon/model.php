...
if (!class_exists("CouponModel")) {
	class CouponModel extends PluginModel
	{
		...
		function useRechargeCoupon($log)
		{
			...
			$coupon = pdo_fetch('select enough,backcredit,backmoney,backredpack from ' . tablename('ewei_shop_coupon') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $data['couponid'], ':uniacid' => $_W['uniacid']));
			if (empty($coupon)) {
				return;
			}
