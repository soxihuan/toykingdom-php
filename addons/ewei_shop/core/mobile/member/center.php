...
if ($_W["isajax"]) {
	...
	$order = array("status0" => pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order") . " where openid=:openid and status=0  and uniacid=:uniacid limit 1", $orderparams), "status1" => pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order") . " where openid=:openid and status=1 and refundstate=0 and uniacid=:uniacid limit 1", $orderparams), "status2" => pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order") . " where openid=:openid and status=2 and refundstate=0 and uniacid=:uniacid limit 1", $orderparams), "status4" => pdo_fetchcolumn("select count(*) from " . tablename("ewei_shop_order") . " where openid=:openid and refundstate>0 and uniacid=:uniacid limit 1", $orderparams),);
