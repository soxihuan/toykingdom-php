...
<style type="text/css">
    ...
    .customer_list_head {height:20px; width:94%; background:#fff; padding:10px 3%;border-bottom:1px solid #eaeaea;font-size: 0；}
    .customer_list_head .info {height:20px; width:40%; float:left;  font-size:16px; color:#666; line-height:20px; text-align:left;}
    .customer_list_head .info span {font-size:14px; color:#999;}
    .customer_list_head .num {height:20px; float:right; text-align:right;line-height:20px; font-size:14px; color:#999;margin-top: 0;padding-top: 0;}
    ...
    .noCustomer{ display: block; margin: 0 auto;height: 131px; width: 207px; background: url('../addons/ewei_shop/template/mobile/default/static/images/map.png') no-repeat center center;background-size: cover;}
</style>
...
<script id='tpl_customer' type='text/html'>
    ...
    <div class="customer_list">
        <div class="img"><img src="<%if user.avatar%><%user.avatar%><%else%>../addons/ewei_shop/plugin/commission/images/head.jpg<%/if%>"></div>
        <div class="left">
        <div class="info"><%if user.nickname%><%user.nickname%><%else%>???<%/if%></div>
        <%if user.weixin%>
            <div class="info"><span>微信号: <%user.weixin%></span></div>
        <%/if%>
        <div class="info"><span>进店时间: <%user.createtime%></span></div>
        </div>
...
<script id='tpl_empty' type='text/html'>
    <div class="customer_no"><i class="noCustomer" style=""></i><br><br><br><span style="line-height:18px; font-size:16px;">还没有人呢~</span></div>
</script>
