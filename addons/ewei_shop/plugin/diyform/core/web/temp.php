...
if ($operation == 'display') {
    ...
    $items  = pdo_fetchall('SELECT * FROM ' . tablename('ewei_shop_diyform_type') . ' WHERE uniacid=:uniacid and title like :name order by id desc limit ' . ($pindex - 1) * $psize . ',' . $psize, array(
        ':name' => "%{$kw}%",
        ':uniacid' => $_W['uniacid']
    ));
    $total  = pdo_fetchcolumn("SELECT COUNT(*) FROM " . tablename('ewei_shop_diyform_type') . " WHERE uniacid=:uniacid and title like :name order by id desc ", array(
        ':uniacid' => $_W['uniacid'],
        ':name' => "%{$kw}%"
    ));
    ...
    if ($_W['ispost']) {
        ...
        if (empty($id)) {
            ...
            pdo_update('ewei_shop_diyform_type', $insert, array(
                'id' => $id
            ));
    ...
    if (!empty($id)) {
        ...
        if ($use_flag1 || $use_flag2 || $datacount3) {
            ...
            pdo_delete('ewei_shop_diyform_type', array(
                'id' => $id
            ));
            $ret = "删除成功";
            show_json(1, $ret);
...
$category = pdo_fetchall('select * from ' . tablename('ewei_shop_diyform_category') . ' where uniacid=:uniacid order by id desc', array(
    ':uniacid' => $_W['uniacid']
), 'id');
load()->func('tpl');
