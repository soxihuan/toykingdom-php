...
var myecho;
    ...
    var rmid =  <?php  echo json_encode($_W['shopshare']['link'])?>;
        function getUrlParam(name) {
            var reg = new RegExp("(^|&)" + name + "=([^&]*)(&|$)");
            var rmidnum = rmid.split('?');
            var atersub = rmidnum[1];
            var r = atersub.match(reg); 
            if (r != null) return unescape(r[2]); return null;
        }
        var mymid=getUrlParam('mid');
        truemid = '&mid='+mymid;
      });
  myecho = "<?php  echo m('user')->getOpenid()?>"; // 使用已登录的 OpenID 来设置用户 ID。
  console.log(myecho)
</script>
