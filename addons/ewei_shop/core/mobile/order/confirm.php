...
$BuyShop = set_medias(m('common')->getSysset('shop'), 'logo');
if (p("commission")) {
		...
		if ($opencommission) {
			...
			if (!empty($mid)) {
				if (empty($cset['closemyshop'])) {
					$BuyShop = set_medias(p('commission')->getShop($mid), 'logo');
				}
			}
		}
	}
...
if ($_W["isajax"]) {
	if ($operation == "display") {
		...
		$member = m("member")->getMember($openid);
		$level = $level_info = m("member")->getLevel($openid);
		$weight = 0;
		$total = 0;
		$goodsprice = 0;
		$realprice = 0;
		...
		$order = array("uniacid" => $uniacid, "openid" => $openid, "ordersn" => $ordersn, "price" => $totalprice, "cash" => $cash, "discountprice" => $discountprice, "deductprice" => $deductmoney, "deductcredit" => $deductcredit, "deductcredit2" => $deductcredit2, "deductenough" => $deductenough, "status" => 0, "paytype" => 0, "transid" => '', "remark" => $_GPC["remark"],"remarkForConsumer" => $_GPC["remark"], "addressid" => empty($dispatchtype) ? $addressid : 0, "goodsprice" => $goodsprice, "dispatchprice" => $dispatch_price, "dispatchtype" => $dispatchtype, "dispatchid" => $dispatchid, "storeid" => $carrierid, "carrier" => $carriers, "createtime" => time(), "isverify" => $isverify ? 1 : 0, "verifycode" => $verifycode, "virtual" => $virtualid, "isvirtual" => $isvirtual ? 1 : 0, "oldprice" => $totalprice, "olddispatchprice" => $dispatch_price, "couponid" => $couponid, "couponprice" => $couponprice);
		if ($diyform_plugin) {
