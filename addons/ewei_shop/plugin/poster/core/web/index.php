...
if ($operation == "display") {
	...
	if (checksubmit("submit")) {
		...
		if (empty($rule)) {
			...
			$rid = pdo_insertid();
			$keyword_data = array("uniacid" => $_W["uniacid"], "rid" => $rid, "module" => "ewei_shop", "content" => $data["type"] == 3 ? ("^" . trim($data["keyword"]) . "\\+*[0-9]{1,}\$") : trim($data["keyword"]), "type" => $data["type"] == 3 ? 3 : 1, "displayorder" => 0, "status" => 1);
		...
		$ruleauto = pdo_fetch("select * from " . tablename("rule") . " where uniacid=:uniacid and module=:module and name=:name  limit 1", array(":uniacid" => $_W["uniacid"], ":module" => "ewei_shop", ":name" => "ewei_shop:poster:auto"));
		if (empty($ruleauto)) {
			$rule_data = array("uniacid" => $_W["uniacid"], "name" => "ewei_shop:poster:auto", "module" => "ewei_shop", "displayorder" => 0, "status" => 1);
			pdo_insert("rule", $rule_data);
			$rid = pdo_insertid();
			$keyword_data = array("uniacid" => $_W["uniacid"], "rid" => $rid, "module" => "ewei_shop", "content" => "ewei_shop_POSTER", "type" => 1, "displayorder" => 0, "status" => 1);
			pdo_insert("rule_keyword", $keyword_data);
		}
