...
if ($operation == 'display') {
	...
	$id = intval($_GPC['id']);
	$item = pdo_fetch("SELECT id,goodsid FROM " . tablename('ewei_shop_order_comment') . " WHERE id ='$id'");
	if (empty($item)) {
		message('抱歉，评价不存在或是已经被删除！', $this->createWebUrl('shop/comment', array('op' => 'display')), 'error');
	}
	pdo_update("ewei_shop_order_comment", array("deleted" => 1), array("id" => $id, 'uniacid' => $_W['uniacid']));
	...
	message("删除成功！", $this->createWebUrl('shop/comment', array('op' => 'display')), 'success');
