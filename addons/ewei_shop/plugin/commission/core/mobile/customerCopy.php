...
$condition = '';
$total = pdo_fetchcolumn("select count(id) from " . tablename("ewei_shop_member") . " where agentid=:agentid and ((isagent=1 and status=0) or isagent=0) and uniacid=:uniacid limit 1", array(":uniacid" => $_W["uniacid"], ":agentid" => $member["id"]));
if ($_W["isajax"]) {
	$pindex = max(1, intval($_GPC["page"]));
	$psize = 20;
	$list = array();
	$sql = "select * from " . tablename("ewei_shop_member") . " where agentid={$member["id"]} and ((isagent=1 and status=0) or isagent=0) and uniacid = " . $_W["uniacid"] . " {$condition}  ORDER BY id desc limit " . ($pindex - 1) * $psize . "," . $psize;
...
include $this->template("customerCopy");