...
$num = pdo_fetchcolumn("select ifnull(count(*),0) from " . tablename("ewei_shop_coupon_data") . " where couponid=:couponid and openid=:openid and uniacid=:uniacid and used=0 ", array(":couponid" => $coupon["id"], ":openid" => $openid, ":uniacid" => $_W["uniacid"]));
