...
if ($operation == 'display') {
    ...
    $psize          = 100;
    $type           = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_diyform_type') . ' WHERE id=:id and uniacid=:uniacid ', array(
        ':id' => $typeid,
        ':uniacid' => $_W['uniacid']
    ));
    ...
    $item = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_diyform_type') . ' WHERE id=:id and uniacid=:uniacid ', array(
        ':id' => $_GPC['typeid'],
        ':uniacid' => $_W['uniacid']
    ));
    if (!empty($item)) {
    ...
    if (!empty($editid)) {
        $data         = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_diyform_data') . ' WHERE id=:id and typeid=:typeid and uniacid=:uniacid ', array(
            ':id' => $editid,
            ':typeid' => $typeid,
            ':uniacid' => $_W['uniacid']
        ));
        ...
        if (!empty($typeid)) {
            ...
            if (!empty($item['fields'])) {
                ...
                foreach ($tpids as $index => $id) {
                    ...
                    $datas  = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_diyform_data') . ' WHERE id=:id and typeid=:typeid and uniacid=:uniacid ', array(
                        ':id' => $id,
                        ':typeid' => $typeid,
                        ':uniacid' => $_W['uniacid']
                    ));
                    if (empty($datas)) {
                        $keydata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_diyform_data') . ' WHERE pvalue=:pvalue  and typeid=:typeid and uniacid=:uniacid ', array(
                            ':pvalue' => $insert['pvalue'],
                            ':typeid' => $typeid,
                            ':uniacid' => $_W['uniacid']
                        ));
                        if (empty($keydata)) {
                            pdo_insert('ewei_shop_diyform_data', $insert);
                            pdo_update('ewei_shop_diyform_type', 'alldata=alldata+1', array(
                                'id' => $item['id']
                            ));
                        } else {
                            pdo_update('ewei_shop_diyform_data', $insert, array(
                                'id' => $keydata['id']
                            ));
                        }
                    } else {
                        $keydata = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_diyform_data') . ' WHERE pvalue=:pvalue and id<>:id and typeid=:typeid and uniacid=:uniacid ', array(
                            ':pvalue' => $insert['pvalue'],
                            ':id' => $id,
                            ':typeid' => $typeid,
                            ':uniacid' => $_W['uniacid']
    ...
    $typeid = intval($_GPC['typeid']);
    if (!empty($id)) {
        $type = pdo_fetch('SELECT * FROM ' . tablename('ewei_shop_diyform_data') . ' WHERE id=:id and uniacid=:uniacid ', array(
            ':id' => $id,
            ':uniacid' => $_W['uniacid']
        ));
        ...
        pdo_delete('ewei_shop_diyform_data', array(
            'id' => $id
        ));
        pdo_update('ewei_shop_diyform_type', 'alldata=alldata-1', array(
            'id' => $typeid
