...
<style type="text/css">
    ...
    .info_main {height:auto;  background:#fff; margin-top:8px; border-bottom:1px solid #e8e8e8; border-top:1px solid #e8e8e8;}
    .info_main .line {margin:0 10px; height:44px; border-bottom:1px solid #e8e8e8; line-height:44px; color:#222121;font-size:16px;}
    .info_main .line .title {height:44px; width:80px; line-height:44px; color:#444; float:left; font-size:16px;}
    .info_main .line .info { width:100%;float:right;margin-left:-80px; }
    .info_main .line .inner { margin-left:80px; }
    .info_main .line .inner input {height:44px; width:100%;display:block; padding:0px; margin:0px; border:0px; float:left; font-size:16px;}
    .info_main .line .inner .user_sex {line-height:40px;}
    .info_sub {height:35px; margin:14px 5px; background:#F4D500; border-radius:4px; text-align:center; font-size:16px; line-height:35px; color:#222121;border:1px solid #222121;}
    .select { border:1px solid #ccc;height:25px;border-radius:3px;}
    input{font-family:微软雅黑;}
    .sex span{color:#999;display:inline-block;height:44px;position:relative;padding-left:27px;}
    .choosesex{display:inline-block;width:20px;height:44px;border-radius:50%;position:absolute;left:0;top:0;}
    .imthis{background:url('../addons/ewei_shop/template/mobile/default/static/images/choose.png') no-repeat center center;background-size:contain;}
    .imNotthis{background:url('../addons/ewei_shop/template/mobile/default/static/images/not-to-choose.png') no-repeat center center;background-size:contain;}
    .blank-angle{color:#000;}
</style>
...
<script id="member_info" type="text/html">
    ...
    <a href="javascript:;" class="back" onclick="history.back()"><i class="fa fa-angle-left blank-angle"></i></a>
    ...
    <div class="info_main">
        ...
        <div class="line sex">
            ...
            <span class="gender" data-val="1"><i class="choosesex <%if gender=='1'%>imthis<%else%>imNotthis<%/if%>" <%if gender=='1'%>style="color:#0C9;"<%/if%>></i> 男</span>&nbsp;&nbsp;
            <span class="gender" data-val="2"><i class="choosesex <%if gender=='2'%>imthis<%else%>imNotthis<%/if%>" <%if gender=='2'%>style="color:#0C9;"<%/if%>></i> 女
        ...
        core.json('member/info',{},function(json){
            if (json.result.member) {
                ...
                $('.gender').click(function() {
                    ...
                    $('.gender').find('i').css('color', '#999').removeClass('imthis').addClass('imNotthis');
                    $(this).find('i').removeClass('imNotthis').addClass('imthis').css('color', '#0c9');
                    $('#gender').val(val);
                })
