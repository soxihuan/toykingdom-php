...
$printTemp1 = pdo_fetchall("SELECT id,type,isdefault,expressname FROM " . tablename("ewei_shop_exhelper_express") . " WHERE type=1 and uniacid=:uniacid ", array(":uniacid" => $_W["uniacid"]));
$printTemp2 = pdo_fetchall("SELECT id,type,isdefault,expressname FROM " . tablename("ewei_shop_exhelper_express") . " WHERE type=2 and uniacid=:uniacid ", array(":uniacid" => $_W["uniacid"]));
if ($op == "search") {
	if ($type == 1) {
