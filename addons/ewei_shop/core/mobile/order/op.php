...
if ($_W["isajax"]) {
	if ($operation == "cancel") {
		...
		$order = pdo_fetch("select id,ordersn,openid,status,deductcredit,deductprice,couponid from " . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
		if (empty($order)) {
			show_json(0, "订单未找到!");
		}
		if ($order["status"] != 0) {
		...
		pdo_update("ewei_shop_order", array("status" => -1, "canceltime" => time()), array("id" => $order["id"], "uniacid" => $uniacid));
		...
		$orderid = intval($_GPC["orderid"]);
		$order = pdo_fetch("select id,status,openid,couponid,refundstate,refundid from " . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
		if (empty($order)) {
			show_json(0, "订单未找到!");
		}
		if ($order["status"] != 2) {
		...
		pdo_update("ewei_shop_order", array("status" => 3, "finishtime" => time(), "refundstate" => 0), array("id" => $order["id"], "uniacid" => $uniacid));
		...
		if ($_W["ispost"]) {
			...
			$orderid = intval($_GPC["orderid"]);
			$order = pdo_fetch('select id,status,price,refundid,goodsprice,dispatchprice,deductprice,deductcredit2,finishtime,isverify,virtual,refundstate from ' . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
			if (empty($order)) {
				show_json(0, "订单未找到!");
			}
		...
		$orderid = intval($_GPC["orderid"]);
		$order = pdo_fetch("select id,status,refundstate,refundid from " . tablename("ewei_shop_order") . " where id=:id and uniacid=:uniacid and openid=:openid limit 1", array(":id" => $orderid, ":uniacid" => $uniacid, ":openid" => $openid));
		if (empty($order)) {
			show_json(0, "订单未找到!");
		}
