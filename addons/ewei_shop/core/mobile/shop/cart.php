...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$goods = pdo_fetch('select id,marketprice,maxbuy from ' . tablename('ewei_shop_goods') . ' where uniacid=:uniacid and id=:id limit 1', array(':uniacid' => $uniacid, ':id' => $id));
		if (empty($goods)) {
			show_json(0, '商品未找到');
		}
        ...
        $datatotal = pdo_fetchall("select sum(total)  from " . tablename('ewei_shop_member_cart') . ' where openid=:openid and goodsid=:id  and deleted=0 and  uniacid=:uniacid   limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':id' => $id));
        ...
        }else if ($goods['maxbuy'] > $datatotal[0]['sum(total)']) {
            if($datatotal[0]['sum(total)'] + $total > $goods['maxbuy']){
                $total = $goods['maxbuy'] - $datatotal[0]['sum(total)'];
            }
            $data = array('uniacid' => $uniacid, 'openid' => $openid, 'goodsid' => $id, 'optionid' => $optionid, 'marketprice' => $goods['marketprice'], 'total' => $total, 'diyformid' => $diyformid, 'diyformdata' => $diyformdata, 'diyformfields' => $diyformfields, 'createtime' => time());
            pdo_insert("ewei_shop_member_cart", $data);
            $cartcount += $total;
