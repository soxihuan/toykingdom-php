...
if (checksubmit("submit")) {
	...
	if (!empty($coverkeyword)) {
		$rule = pdo_fetch("select * from " . tablename("rule") . " where uniacid=:uniacid and module=:module and name=:name limit 1", array(":uniacid" => $_W["uniacid"], ":module" => "cover", ":name" => "ewei_shop优惠券入口设置"));
		if (!empty($rule)) {
		...
		$rule_data = array("uniacid" => $_W["uniacid"], "name" => "ewei_shop优惠券入口设置", "module" => "cover", "displayorder" => 0, "status" => $status);
		if (empty($rule)) {
