...
if (!class_exists('DiyformModel')) {
    class DiyformModel extends PluginModel
    {
        ...
        public function getLastOrderData($diyformid, $member)
        {
            ...
            $f_data = array();
            if (!empty($diyformid)) {
                $order = pdo_fetch('select diyformdata,diyformfields from ' . tablename('ewei_shop_order') . " where diyformid=:diyformid and openid=:openid order by id desc limit 1", array(
                    ':diyformid' => $diyformid,
                    ':openid' => $member['openid']
                ));
                if (!empty($order)) {
        ...
        {
            global $_W;
            $sql    = 'select * from ' . tablename('ewei_shop_diyform_temp') . " where cid=:cid and diyformid=:diyformid and openid=:openid and uniacid=:uniacid Limit 1";
            $params = array(
                ':cid' => $goodsid,
                ':diyformid' => $diyformid,
                ':openid' => $openid,
        ...
        {
            global $_W;
            $form_list = pdo_fetchall('select * from ' . tablename('ewei_shop_diyform_type') . ' where uniacid=:uniacid', array(
                ':uniacid' => $_W['uniacid']
            ));
            return $form_list;
        }
        ...
        {
            global $_W;
            $formInfo = pdo_fetch('select * from ' . tablename('ewei_shop_diyform_type') . ' where id=:id and uniacid=:uniacid Limit 1', array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
        ...
        {
            global $_W;
            $data = pdo_fetch('select * from ' . tablename('ewei_shop_diyform_data') . ' where id=:id and uniacid=:uniacid limit 1', array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
            if (!empty($data) && $flag) {
        ...
        {
            global $_W;
            $data = pdo_fetch('select * from ' . tablename('ewei_shop_diyform_temp') . ' where id=:id and uniacid=:uniacid Limit 1', array(
                ':id' => $id,
                ':uniacid' => $_W['uniacid']
            ));
            if (!empty($data) && $flag) {
