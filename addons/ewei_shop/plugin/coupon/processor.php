...
class CouponProcessor extends PluginProcessor
{
	...
	function getGuess($_var_5, $_var_12)
	{
		...
		$_var_8 = 1;
		$_var_7 = 0;
		$_var_13 = pdo_fetch("select id,times from " . tablename("ewei_shop_coupon_guess") . " where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and uniacid=:uniacid limit 1 ", array(":couponid" => $_var_5["id"], ":openid" => $_var_12, ":pwdkey" => $_var_5["pwdkey"], ":uniacid" => $_W["uniacid"]));
	...
	function respondText($_var_0)
	{
		...
		if (isset($_SESSION["ewei_shop_coupon_key"])) {
			$_var_14 = $_SESSION["ewei_shop_coupon_key"];
		} else {
			$_SESSION["ewei_shop_coupon_key"] = $_var_2;
		}
		$_var_5 = pdo_fetch('select id,couponname,pwdkey,pwdask,pwdsuc,pwdfail,pwdfull,pwdtimes,pwdurl,pwdwords,pwdown,pwdexit,pwdexitstr from ' . tablename("ewei_shop_coupon") . " where pwdkey=:pwdkey and uniacid=:uniacid limit 1", array(":uniacid" => $_W["uniacid"], ":pwdkey" => $_var_14));
		...
		if (!$_var_0->inContext) {
			$_var_16 = pdo_fetch("select id,times from " . tablename("ewei_shop_coupon_guess") . ' where couponid=:couponid and openid=:openid and pwdkey=:pwdkey and ok=1 and uniacid=:uniacid limit 1 ', array(":couponid" => $_var_5["id"], ":openid" => $_var_12, ":pwdkey" => $_var_5["pwdkey"], ":uniacid" => $_W["uniacid"]));
			if (!empty($_var_16)) {
			...
			if (empty($_var_13)) {
				...
				pdo_insert("ewei_shop_coupon_guess", $_var_13);
			} else {
				pdo_update("ewei_shop_coupon_guess", array("times" => $_var_13["times"] + 1, "ok" => $_var_17 ? 1 : 0), array("id" => $_var_13["id"]));
			}
			$_var_18 = time();
			if ($_var_17) {
