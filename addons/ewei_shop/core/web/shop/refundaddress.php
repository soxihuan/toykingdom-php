...
if ($operation == "display") {
	...
	$list = pdo_fetchall("SELECT * FROM " . tablename("ewei_shop_refund_address") . " WHERE uniacid = '{$_W["uniacid"]}' and deleted = 0 ORDER BY id asc");
	...
	if (checksubmit("submit")) {
		...
		if ($data["isdefault"]) {
			pdo_update("ewei_shop_refund_address", array("isdefault" => 0), array("uniacid" => $_W["uniacid"]));
		}
		if (!empty($id)) {
	...
	if (!empty($id)) {
		$item = pdo_fetch("SELECT * FROM " . tablename("ewei_shop_refund_address") . " WHERE id = '$id' and uniacid = '{$_W["uniacid"]}'");
		if (!empty($item)) {
		}
	}
	...
	$id = intval($_GPC["id"]);
	$item = pdo_fetch("SELECT id,title FROM " . tablename("ewei_shop_refund_address") . " WHERE id = '$id' AND uniacid=" . $_W["uniacid"] . "");
	if (empty($item)) {
		message("抱歉，退货地址不存在或是已经被删除！", $this->createWebUrl("shop/refundaddress", array("op" => "display")), "error");
	}
	pdo_delete("ewei_shop_refund_address", array("id" => $id));
