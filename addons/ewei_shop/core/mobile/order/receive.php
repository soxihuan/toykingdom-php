...
foreach ($sets as $set) {
	...
	$p = p("commission");
	$pcoupon = p("coupon");
	$orders = pdo_fetchall("select id,couponid from " . tablename("ewei_shop_order") . " where uniacid={$_W["uniacid"]} and status=2 and sendtime + {$daytimes} <=unix_timestamp() ", array(), "id");
	if (!empty($orders)) {
		$orderkeys = array_keys($orders);
		$orderids = implode(",", $orderkeys);
		if (!empty($orderids)) {
			pdo_query("update " . tablename("ewei_shop_order") . " set status=3,finishtime=" . time() . " where id in (" . $orderids . ")");
